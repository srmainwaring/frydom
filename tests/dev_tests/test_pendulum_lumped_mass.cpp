//
// Created by frongere on 09/04/2020.
//
...
int main() {
  ...
  FrOffshoreSystem system("test_pendulum_lumped_mass");
...
//  auto anchor = seabed->NewAnchor("anchor", -500., 0., NWU);
  ...
  anchor->SetPositionInWorld({0, 0, 0}, NWU);
...
//  double cable_length = 64 + 425;
  double cable_length = 50; // Adding more length...
...
//  auto cable_properties = make_cable_properties(diameter, linear_density, E, rayleighDamping);
  ...
  auto cat_cable = make_catenary_line("catenary_cable",
                                      anchor,
                                      cylinder_anchor,
                                      cable_properties,
                                      true,
                                      cable_length,
                                      WATER); // TODO: devrait y avoir une detection auto du fluide...
  cat_cable->Initialize();

  Position pL = cat_cable->GetPositionInWorld(cable_length, NWU);

  auto cat_cable_bis = make_catenary_line("catenary_cable_bis",
                                          anchor,
                                          cylinder_anchor,
                                          cable_properties,
                                          true,
                                          cable_length,
                                          WATER); // TODO: devrait y avoir une detection auto du fluide...
  cat_cable_bis->Initialize();

  auto tension_anchor = cat_cable->GetTension(0., NWU).norm();
  auto tension_cylinder = cat_cable->GetTension(cable_length, NWU).norm();
//
...
//                                 nb_elements);
  ...
  auto cable = make_lumped_mass_cable("cable",
                                      anchor,
                                      cylinder_anchor,
                                      cable_properties,
                                      cable_length,
