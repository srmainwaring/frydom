...
- When using `SwapFrameConvention` it is recommended to use the return value of the function instead the argument vector since error on execution has been observed when no return value is used.
...
- The `Initialize` method of `FrOffshoreSystem` has been modified to be consistent with the modification of `chrono::ChSystem`. `SetupInitialize`is used instead `ExecuteControlforUpdate`.
