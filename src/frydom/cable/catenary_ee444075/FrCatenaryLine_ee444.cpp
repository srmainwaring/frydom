...
#include "FrCatenaryLine_ee444.h"
...
namespace frydom {
  ...
  FrCatenaryLine_ee444::FrCatenaryLine_ee444(const std::string &name,
      ...
      FrPhysicsItem(),
      FrCable_ee444(startingNode, endingNode, properties, unstrainedLength),
      m_elastic(elastic),
      c_fluid(fluid) {
  ...
  void FrCatenaryLine_ee444::DefineLogMessages() {
  ...
  void FrCatenaryLine_ee444::SetSolverTolerance(double tol) {
    m_tolerance = tol;
  }

  void FrCatenaryLine_ee444::SetSolverMaxIter(unsigned int maxiter) {
    m_itermax = maxiter;
  }

  void FrCatenaryLine_ee444::SetSolverInitialRelaxFactor(double relax) {
    m_relax = relax;
  }

  void FrCatenaryLine_ee444::guess_tension() {
  ...
  Force FrCatenaryLine_ee444::GetTension(const double& s, FRAME_CONVENTION fc) const {
    Force tension = m_t0 - c_qvec * s;
    if (IsNED(fc)) { internal::SwapFrameConvention(tension); }
    return tension;
  }

  std::shared_ptr<FrCatenaryForce_ee444> FrCatenaryLine_ee444::GetStartingForce() {
    return m_startingForce;
  }

  std::shared_ptr<FrCatenaryForce_ee444> FrCatenaryLine_ee444::GetEndingForce() {
    return m_endingForce;
  }

  Force FrCatenaryLine_ee444::GetStartingNodeTension(FRAME_CONVENTION fc) const {
    return GetTension(0., fc);
  }

  Force FrCatenaryLine_ee444::GetEndingNodeTension(FRAME_CONVENTION fc) const {
    return -GetTension(GetUnstrainedLength(), fc);
  }

  double FrCatenaryLine_ee444::_rho(double s) const {
    auto t0_qS = GetTension(s, NWU);
    return t0_qS.norm() - m_u.dot(t0_qS);
  }

  double FrCatenaryLine_ee444::GetUnstrainedLength() const {
    return m_unstrainedLength;
  }

  Position FrCatenaryLine_ee444::GetUnstrainedChord(double s, FRAME_CONVENTION fc) const {
  ...
  Position FrCatenaryLine_ee444::GetElasticIncrement(double s, FRAME_CONVENTION fc) const {
    Position Inc(0., 0., 0.);
  ...
  Position FrCatenaryLine_ee444::GetNodePositionInWorld(double s, FRAME_CONVENTION fc) const {
  ...
  Position FrCatenaryLine_ee444::get_residual(FRAME_CONVENTION fc) const {
    return GetNodePositionInWorld(m_unstrainedLength, fc) - GetEndingNode()->GetPositionInWorld(fc);
  }

  mathutils::Matrix33<double> FrCatenaryLine_ee444::analytical_jacobian() const {
    auto t0n = m_t0.norm();
  ...
  void FrCatenaryLine_ee444::solve() {
  ...
  void FrCatenaryLine_ee444::Initialize() {
    ...
    if (!m_startingForce) {
      m_startingForce = std::make_shared<FrCatenaryForce_ee444>(GetName() + "_start_force", m_startingNode->GetBody(), this,
                                                          LINE_START);
    ...
    if (!m_endingForce) {
      m_endingForce = std::make_shared<FrCatenaryForce_ee444>(GetName() + "_end_force", m_endingNode->GetBody(), this,
                                                        LINE_END);
  ...
  void FrCatenaryLine_ee444::Compute(double time) {
  ...
  void FrCatenaryLine_ee444::UpdateState() {
    FrCable_ee444::UpdateState();
    solve();
  }
  ...
  internal::FrPhysicsItemBase *FrCatenaryLine_ee444::GetChronoItem_ptr() const { return m_chronoPhysicsItem.get(); }


  std::shared_ptr<FrCatenaryLine_ee444>
  make_catenary_line_ee444(const std::string &name,
                     const std::shared_ptr<FrNode> &startingNode,
                     const std::shared_ptr<FrNode> &endingNode,
                     const std::shared_ptr<FrCableProperties> &properties,
                     bool elastic,
    ...
    auto CatenaryLine = std::make_shared<FrCatenaryLine_ee444>(name,
                                                         startingNode,
                                                         endingNode,
                                                         properties,
                                                         elastic,
